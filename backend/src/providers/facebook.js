...
function mapFacebookError(error, fallbackCode) {
  ...
  if (lower.includes('private') || lower.includes('login required')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'Facebook content is restricted',
      'This content may be private or require login.'
    );
  }
...
const facebookProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapFacebookError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'facebook_media');
    ...
    try {
      ...
      throw mapFacebookError(error, 'DOWNLOAD_FAILED');
    }
  }
};
