...
function mapOkruError(error, fallbackCode) {
  const text = String(error?.message || error || '');
  const lower = text.toLowerCase();
  if (lower.includes('private') || lower.includes('not available')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'OK.ru content is restricted',
...
const okruProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapOkruError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'okru_media');
    ...
    try {
      ...
      throw mapOkruError(error, 'DOWNLOAD_FAILED');
    }
  }
};
