...
function isNineGagUrl(url) {
  try {
    const parsed = new URL(url);
    return parsed.hostname.toLowerCase().includes('9gag.com');
  } catch (_) {
    return false;
  }
}
...
const ninegagProvider = {
  id: '9gag',

  match(url) {
    return isNineGagUrl(url);
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || '9gag_media');
