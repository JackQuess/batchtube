...
function isRutubeUrl(url) {
  try {
    ...
    return parsed.hostname.toLowerCase().includes('rutube.ru');
  } catch (_) {
    return false;
  }
}

function mapRutubeError(error, fallbackCode) {
  const text = String(error?.message || error || '');
  const lower = text.toLowerCase();
  if (lower.includes('not available') || lower.includes('private')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'RuTube content is restricted',
...
const rutubeProvider = {
  id: 'rutube',

  match(url) {
    return isRutubeUrl(url);
  },
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapRutubeError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'rutube_media');
    ...
    try {
      ...
      throw mapRutubeError(error, 'DOWNLOAD_FAILED');
    }
  }
};
