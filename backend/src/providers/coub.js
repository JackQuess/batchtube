...
function isCoubUrl(url) {
  try {
    const parsed = new URL(url);
    return parsed.hostname.toLowerCase().includes('coub.com');
  } catch (_) {
    return false;
  }
}
...
const coubProvider = {
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'coub_media');
