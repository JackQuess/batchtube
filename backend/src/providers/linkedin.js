...
function mapLinkedInError(error, fallbackCode) {
  ...
  if (lower.includes('login required') || lower.includes('private')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'LinkedIn content is restricted',
      'This video may require login or may be private.'
    );
  }
...
const linkedinProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapLinkedInError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'linkedin_media');
    ...
    try {
      ...
      throw mapLinkedInError(error, 'DOWNLOAD_FAILED');
    }
  }
};
