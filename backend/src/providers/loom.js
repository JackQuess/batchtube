...
function isLoomUrl(url) {
  try {
    ...
    return parsed.hostname.toLowerCase().includes('loom.com');
  } catch (_) {
    return false;
  }
}

function mapLoomError(error, fallbackCode) {
  const text = String(error?.message || error || '');
  const lower = text.toLowerCase();
  if (lower.includes('private') || lower.includes('not found')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'Loom content is restricted',
      'This recording may be private or unavailable.'
    );
  }
...
const loomProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapLoomError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'loom_media');
    ...
    try {
      ...
      throw mapLoomError(error, 'DOWNLOAD_FAILED');
    }
  }
};
