...
function isSoundCloudUrl(url) {
  try {
    ...
    return parsed.hostname.toLowerCase().includes('soundcloud.com');
  } catch (_) {
    return false;
  }
}

function mapSoundCloudError(error, fallbackCode) {
  ...
  if (lower.includes('private') || lower.includes('login required')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'SoundCloud content is restricted',
      'This track may be private or require login.'
    );
  }
...
const soundcloudProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapSoundCloudError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'soundcloud_media');
    ...
    try {
      ...
      throw mapSoundCloudError(error, 'DOWNLOAD_FAILED');
    }
  }
};
