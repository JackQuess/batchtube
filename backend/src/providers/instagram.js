...
function mapInstagramError(error, fallbackCode) {
  ...
  if (lower.includes('private') || lower.includes('login required')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'Instagram content is restricted',
      'This post may be private or require login.'
    );
  }
...
const instagramProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapInstagramError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'instagram_media');
    ...
    try {
      ...
      throw mapInstagramError(error, 'DOWNLOAD_FAILED');
    }
  }
};
