...
function isDailymotionUrl(url) {
  try {
    ...
    return host.includes('dailymotion.com') || host.includes('dai.ly');
  } catch (_) {
    return false;
  }
}

function mapDailymotionError(error, fallbackCode) {
  ...
  if (lower.includes('geo') || lower.includes('country')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'Dailymotion content is region restricted',
      'This video may be unavailable in your region.'
    );
  }
...
const dailymotionProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapDailymotionError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'dailymotion_media');
    ...
    try {
      ...
      throw mapDailymotionError(error, 'DOWNLOAD_FAILED');
    }
  }
};
