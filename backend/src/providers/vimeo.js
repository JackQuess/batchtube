...
function mapVimeoError(error, fallbackCode) {
  const text = String(error?.message || error || '');
  const lower = text.toLowerCase();

  if (lower.includes('password') || lower.includes('private')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'Vimeo content is restricted',
      'This video may be private or password-protected.'
    );
  }
...
const vimeoProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapVimeoError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'vimeo_media');
    ...
    try {
      ...
      throw mapVimeoError(error, 'DOWNLOAD_FAILED');
    }
  }
};
