...
function isTwitchUrl(url) {
  try {
    ...
    return host.includes('twitch.tv') || host.includes('clips.twitch.tv');
  } catch (_) {
    return false;
  }
}

function mapTwitchError(error, fallbackCode) {
  const text = String(error?.message || error || '');
  const lower = text.toLowerCase();

  if (lower.includes('subscriber') || lower.includes('login required')) {
    return new ProviderError(
      'NEEDS_VERIFICATION',
      text || 'Twitch content requires verification',
      'This clip/VOD may require login or subscription.'
    );
  }

  if (lower.includes('restricted') || lower.includes('unavailable')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'Twitch content is restricted',
      'This clip/VOD may be unavailable.'
    );
  }
...
const twitchProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapTwitchError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'twitch_media');
    ...
    try {
      ...
      throw mapTwitchError(error, 'DOWNLOAD_FAILED');
    }
  }
};
