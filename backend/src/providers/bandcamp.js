...
function isBandcampUrl(url) {
  try {
    ...
    return parsed.hostname.toLowerCase().includes('bandcamp.com');
  } catch (_) {
    return false;
  }
}

function mapBandcampError(error, fallbackCode) {
  const text = String(error?.message || error || '');
  const lower = text.toLowerCase();

  if (lower.includes('not available') || lower.includes('streaming not allowed')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'Bandcamp content is restricted',
      'This track may not be available for streaming/download.'
    );
  }
...
const bandcampProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapBandcampError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'bandcamp_media');
    ...
    try {
      ...
      throw mapBandcampError(error, 'DOWNLOAD_FAILED');
    }
  }
};
