...
function mapVkError(error, fallbackCode) {
  const text = String(error?.message || error || '');
  const lower = text.toLowerCase();

  if (lower.includes('private') || lower.includes('friends only')) {
    return new ProviderError(
      'RESTRICTED',
      text || 'VK content is restricted',
      'This video may be private or access-limited.'
    );
  }
...
const vkProvider = {
  ...
  async getMetadata(url) {
    try {
      ...
      throw mapVkError(error, 'METADATA_FAILED');
    }
  },
  ...
  async download(url, opts) {
    ...
    const baseName = sanitizeFilename(opts.baseName || 'vk_media');
    ...
    try {
      ...
      throw mapVkError(error, 'DOWNLOAD_FAILED');
    }
  }
};
