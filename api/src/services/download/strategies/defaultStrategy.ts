...
import { resolveGenericCookieModes } from '../cookies.js';
...
export function buildDefaultFlatAttempts(
  url: string,
...
export async function downloadDefaultStrategy(input: FlatDownloadInput): Promise<{
  filePath: string;
  mimeType: string;
  ext: string;
}> {
  const provider = input.provider || 'generic';
  const attempts = buildDefaultFlatAttempts(input.url, input.format, input.quality);
  return runFlatAttemptDownload({ ...input, provider }, attempts, { useInstagramBackoff: false });
}
