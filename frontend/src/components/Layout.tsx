...
export const Layout: React.FC<LayoutProps> = ({ children, activeView, onNavigate }) => {
  ...
  return (
    <div className="font-sans bg-background-dark text-white antialiased min-h-screen flex flex-col relative overflow-hidden selection:bg-primary/30 selection:text-white">
      ...
      {!isAuthPage && (
        <header className="relative z-50 w-full px-6 md:px-8 py-6 flex justify-between items-center max-w-7xl mx-auto">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <button onClick={() => onNavigate('landing')} className={`text-sm font-medium transition-colors ${activeView === 'landing' ? 'text-white' : 'text-gray-400 hover:text-white'}`}>Product</button>
            <button onClick={() => onNavigate('pricing')} className={`text-sm font-medium transition-colors ${activeView === 'pricing' ? 'text-white' : 'text-gray-400 hover:text-white'}`}>Pricing</button>
            <button onClick={() => onNavigate('supported-sites')} className={`text-sm font-medium transition-colors ${activeView === 'supported-sites' ? 'text-white' : 'text-gray-400 hover:text-white'}`}>Supported Sites</button>
            <button onClick={() => onNavigate('faq')} className={`text-sm font-medium transition-colors ${activeView === 'faq' ? 'text-white' : 'text-gray-400 hover:text-white'}`}>FAQ</button>
          </nav>
      ...
      {/* Main Content */}
      <main className={`flex-1 flex flex-col relative z-10 ${isAuthPage ? 'items-center justify-center p-4' : ''}`}>
        {children}
      </main>
          ...
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
             <div>
               ...
               <p className="text-gray-500 text-xs leading-relaxed">
                 The ultimate multi-provider batch downloader. Paste links, get ZIPs.
               </p>
             </div>
             <div>
               ...
               <ul className="space-y-2 text-xs text-gray-500">
                 ...
                 <li><button onClick={() => onNavigate('supported-sites')} className="hover:text-primary transition-colors">Supported Sites</button></li>
               </ul>
             </div>
             <div>
               <h4 className="text-white font-medium mb-3">Support</h4>
               <ul className="space-y-2 text-xs text-gray-500">
                 <li><button onClick={() => onNavigate('faq')} className="hover:text-primary transition-colors">FAQ</button></li>
                 <li><button onClick={() => onNavigate('contact')} className="hover:text-primary transition-colors">Contact</button></li>
                 <li><button onClick={() => onNavigate('status')} className="hover:text-primary transition-colors">Status</button></li>
               </ul>
             </div>
             <div>
               <h4 className="text-white font-medium mb-3">Legal</h4>
               <ul className="space-y-2 text-xs text-gray-500">
                 <li><button onClick={() => onNavigate('legal')} className="hover:text-primary transition-colors">Terms of Service</button></li>
                 <li><button onClick={() => onNavigate('legal')} className="hover:text-primary transition-colors">Privacy Policy</button></li>
                 <li><button onClick={() => onNavigate('legal')} className="hover:text-primary transition-colors">Refund Policy</button></li>
               </ul>
             </div>
          </div>
          <div className="text-center text-xs text-gray-700 border-t border-white/5 pt-6">
            © 2026 BatchTube Inc. Multi-platform downloading engine.
          </div>
        </footer>
      )}
    </div>
...
const SidebarItem: React.FC<{ icon: string; label: string; active?: boolean; onClick?: () => void }> = ({ icon, label, active, onClick }) => (
  ...
  >
    <span className={`material-symbols-outlined text-[20px] transition-colors ${active ? 'text-primary' : 'group-hover:text-white'}`}>{icon}</span>
    {label}
  </button>
);
