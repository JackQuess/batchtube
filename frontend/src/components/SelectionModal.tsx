...
export const SelectionModal: React.FC<SelectionModalProps> = ({
  ...
  return (
    <div 
      className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fadeIn"
      onClick={onClose}
    >
      <div 
        className="w-full max-w-2xl bg-[#0b0b10] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-slideUp"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-6 border-b border-white/5 bg-[#0b0b10] flex items-center justify-between">
          <div>
            ...
            {items.length === 0 ? (
              ...
              items.map((item, index) => (
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-white font-medium text-sm mb-1 truncate" title={item.video.title}>
                    {item.video.title}
                  </h3>
                  <div className="flex items-center gap-3 text-xs text-gray-400">
                    <span>{item.video.channel || 'Unknown Channel'}</span>
                    {item.video.duration && (
                      <>
                ...
                <button
                  ...
                  className="flex-shrink-0 p-2 text-gray-400 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors"
